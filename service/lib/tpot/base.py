...
class TPOTBase(BaseEstimator):
    ...
    def fit(self, features, target, message_queue, sample_weight=None, groups=None):
    ...
    def _evaluate_individuals(self, population, features, target, sample_weight=None, groups=None):
        ...
        self._message_queue.put(f'pbar:{num_population-len(individuals)}')
        if self.verbosity > 0:
    ...
    def _update_pbar(self, pbar_num=1, pbar_msg=None):
        ...
        None
        """
        self._message_queue.put(f'pbar:{pbar_num}')
        if not isinstance(self._pbar, type(None)):
